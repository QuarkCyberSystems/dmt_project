...
def create_personal_id(name):
	...
	if(frappe.get_doc("Device Allocation Tool", apt_name)):
		device_doc=frappe.get_doc("Device Allocation Tool", apt_name)
		device_tab=device_doc.access_control_device
		for d in device_tab:
			control_doc=frappe.get_doc("Access Control Device", d.get("access_control_device"))
			frappe.errprint(control_doc)
		
			device_username=control_doc.user_name
			device_password=control_doc.password
				...
				for i in range(0, len(tab)):
					...
					try:
						if(json_data['errors']):
							...
							to_epoch = calendar.timegm(date_time.timetuple())
...
def set_third_party_push():
	...
	if(frappe.get_all("Access Control Device", filters={'status':'Active'}, fields=["user_name", "password", "url"])):
		doc=frappe.get_all("Access Control Device", filters={'status':'Active'}, fields=["user_name", "password", "url"])
		for i in doc:
	  		
			device_username=i.get("user_name")
			device_password=i.get("password")
			device_url=i.get("url")
			
			session_id, h_pass = megvii_login_challenge(device_username, device_password, device_url)
			status = megvii_device_login(device_username, device_url, session_id, h_pass)
			frappe.errprint("ooooo")
